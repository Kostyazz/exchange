Реализовано консольное приложение биржи для торговли долларами за рубли.
Функции:

- Регистрация заявки на покупку / продажу.
- Просмотр своего баланса
- Отмена заявки по её id
...
IP адрес и порт устанавливаются в файле Common.hpp

Для использования приложения следует запустить Server, после - Client для каждого пользователя.
Выбор функции реализован через бесконечное меню. Пользователь вводит номер функции, затем, при необходимости, вводит дополнительные данные.

Для тестирования функций можно использовать ClientDummy. Эта программа считывает данные из файла test.txt, и выводит в файл out.txt.
